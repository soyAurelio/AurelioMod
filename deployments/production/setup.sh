...
if [ -n "${WEAVIATE_HOSTNAME:-}" ] && [ -n "${WEAVIATE_API_KEY:-}" ]; then
	echo "Creando schema Weaviate Cloud..."
	curl -sf -X POST "$WEAVIATE_HOSTNAME/v1/schema" \
	...
	echo "  ⏭  WEAVIATE_HOSTNAME/API_KEY no definidos — schema manual después"
fi
